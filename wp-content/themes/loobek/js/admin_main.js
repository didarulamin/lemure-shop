...
jQuery(function($){
	...
	$('.ts-gallery-box .add-image').on('click', function(event){
        ...
        var _states = [new wp.media.controller.Library({
            filterable: 'uploaded',
            title: loobek_admin_texts.select_images,
            multiple: true,
            priority:  20
        })];
        ...
        file_frame = wp.media.frames.file_frame = wp.media({
            states: _states,
            button: {
                text: loobek_admin_texts.use_images
            }
        });
...
jQuery(function($){
	...
	if( add_sidebar_form.length > 0 ){
		...
		$('#ts-add-sidebar').on('click', function(e){
			...
			if( sidebar_name != '' ){
				...
				var data = {
					action: 'loobek_add_custom_sidebar'
	...
	if( $('.sidebar-ts-custom-sidebar').length > 0 ){
		...
		$('.sidebar-ts-custom-sidebar .delete-sidebar').on('click', function(){
			...
			var sidebar_nonce = $('#ts_custom_sidebar_nonce').val();
			var ok = confirm( loobek_admin_texts.delete_sidebar_confirm );
			if( ok ){
				widget_block.hide();
				var data = {
					action: 'loobek_delete_custom_sidebar'
					,sidebar_name: sidebar_name
					,sidebar_nonce: sidebar_nonce
				};
				...
				$.ajax({
					...
					,success : function(response){
						if( response != '' ){
							...
							widget_block.show();
							alert( loobek_admin_texts.delete_sidebar_failed );
						}
					}
				});
			}
...
jQuery(function($){
	...
	$( document ).on( 'click', '.ts-product-cat-upload-field .upload-button', function( event ) {
		...
		file_frame = wp.media.frames.downloadable_file = wp.media({
			title: loobek_admin_texts.choose_an_image,
			button: {
				text: loobek_admin_texts.use_image
			},
			multiple: false
		});
...
jQuery(function($){
	...
	function update_post_edit_view_buttons( sel ){
		...
		if( !sel.siblings('.' + edit_button_class).length ){
			sel.after('<a class="' + edit_button_class + '" href="#" target="_blank" style="display: none">' + loobek_admin_texts.edit_post_button_label + '</a>');
		}
		if( !sel.siblings('.' + view_button_class).length ){
			sel.after('<a class="' + view_button_class + '" href="#" target="_blank" style="display: none">' + loobek_admin_texts.view_posts_button_label + '</a>');
		}
		
		if( val != 0 && val != '' ){
			sel.siblings('.' + edit_button_class).attr('href', loobek_admin_texts.edit_post_url_pattern[post_type].replace('[post_id]', val)).show();
			sel.siblings('.' + view_button_class).hide();
		}
		else{
			sel.siblings('.' + view_button_class).attr('href', loobek_admin_texts.view_posts_url_pattern.replace('[post_type]', post_type)).show();
