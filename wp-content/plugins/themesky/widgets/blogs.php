...
if( !class_exists('TS_Blogs_Widget') ){
	class TS_Blogs_Widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			$show_comment 		= function_exists('loobek_get_post_comment_count') ? $show_comment : 0;
			...
			if( $posts->have_posts() ):
				...
				if( $show_author || $show_date || $show_comment || $show_excerpt ){
					$classes[] = 'style-list-item';
				}
						...
						<?php endif; ?>
							<li class="<?php echo $post_format ?>">
								<?php if( $show_thumbnail ): ?>
									<?php if( $post_format == 'gallery' || $post_format === false || $post_format == 'standard' || $post_format == 'video' ){ ?>
										<a class="thumbnail <?php echo esc_attr($post_format); echo ($post_format == 'gallery')?' loading':''; ?>" href="<?php the_permalink(); ?>">
											<figure>
												...
												if( $post_format === false || $post_format == 'standard' || $post_format == 'video' ){
								...
								<div class="entry-content">								
									...
									<?php if( $show_excerpt && function_exists('loobek_the_excerpt_max_words') ): ?>
										<div class="excerpt">
											<?php loobek_the_excerpt_max_words($excerpt_words, $post); ?>
										</div>
									<?php endif; ?>
