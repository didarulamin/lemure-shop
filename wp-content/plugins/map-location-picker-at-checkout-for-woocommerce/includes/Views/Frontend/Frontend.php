...
JAVASCRIPT;
    ...
    private function maybe_create_store_location_selector_fields() : void
    {
        ...
        if ( ($this->get_cost_by_distance_setting() === false || $this->get_cost_by_store_distance_setting() === false) && $this->get_cost_by_store_location_setting() === false && Store_Locations::showStoreSelectorInCheckoutEnabled() === true ) {
    ...
    public function output_map_on_checkout_page()
    {
        ...
        $display = 'block';
        if ( false === Map_Visibility_Controller::lpac_show_map( 'checkout' ) ) {
            $display = 'none';
        }
        ...
        echo  wp_kses_post( $instuctions_text ) ;
        ?></div>
			<?php 
    ...
    {
        if ( Store_Locations::showStoreSelectorInCheckoutEnabled() === false && $this->get_cost_by_store_distance_setting() === false && $this->get_cost_by_store_location_setting() === false ) {
            return;
        }
